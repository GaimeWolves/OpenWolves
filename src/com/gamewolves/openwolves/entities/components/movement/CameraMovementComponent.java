...
public class CameraMovementComponent extends Component
{
	...
	public void updateMovement(TransformComponent transform)
	{
		...
		{
			transform.translate(speed * (float) Math.sin(-rotation.y) * Display.getDeltaTimeInSeconds(), 0,
					speed * -(float) Math.cos(-rotation.y) * Display.getDeltaTimeInSeconds());
		}
		
		if (InputHandler.isKeyDown(GLFW.GLFW_KEY_A))
		{
			transform.translate(speed * (float) Math.sin(-rotation.y - (Math.PI / 2)) * Display.getDeltaTimeInSeconds(),
					0, speed * -(float) Math.cos(-rotation.y - (Math.PI / 2)) * Display.getDeltaTimeInSeconds());
		}
		...
		if (InputHandler.isKeyDown(GLFW.GLFW_KEY_SPACE))
			transform.translate(0, speed * Display.getDeltaTimeInSeconds(), 0);
		else if (InputHandler.isKeyDown(GLFW.GLFW_KEY_C))
			transform.translate(0, -speed * Display.getDeltaTimeInSeconds(), 0);
