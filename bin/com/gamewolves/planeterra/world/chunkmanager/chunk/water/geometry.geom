...
in vec2 geometry_textureCoords[];
...
const vec3 waterColour = vec3(0.6, 0.8, 0.8);
...
void main(void){
	...
	textureCoords = geometry_textureCoords[0];
	EmitVertex();

	worldPosition = gl_in[1].gl_Position;
	gl_Position = projectionMatrix * viewMatrix * worldPosition;
	finalColour = colour + calculateSpecular(worldPosition, normal);
	textureCoords = geometry_textureCoords[1];
	EmitVertex();

	worldPosition = gl_in[2].gl_Position;
	gl_Position = projectionMatrix * viewMatrix * worldPosition;
	finalColour = colour + calculateSpecular(worldPosition, normal);
	textureCoords = geometry_textureCoords[2];
	EmitVertex();
